...
export default function CreateModal({ isOpen, closeModal, onSuccess, item = null }) {
    const { data, setData, reset, clearErrors } = useForm({ nombre: '' });

    useEffect(() => {
        if (isOpen) {
            if (item) setData({ nombre: item.nombre });
            else reset();
            clearErrors();
        }
    }, [isOpen, item]);
    ...
    const submit = (e) => {
        e.preventDefault();
        const promise = item
            ? axios.put(`/api/inventario/marcas/${item.id}`, data)
            : axios.post('/api/inventario/marcas', data);
