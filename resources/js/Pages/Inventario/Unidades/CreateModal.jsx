...
export default function CreateModal({ isOpen, closeModal, onSuccess, item = null }) {
    const { data, setData, reset, clearErrors } = useForm({ nombre: '', abreviatura: '' });

    useEffect(() => {
        if (isOpen) {
            if (item) setData({ nombre: item.nombre, abreviatura: item.abreviatura });
            else reset();
            clearErrors();
        }
    }, [isOpen, item]);
    ...
    const submit = (e) => {
        e.preventDefault();
        const promise = item
            ? axios.put(`/api/inventario/unidades/${item.id}`, data)
            : axios.post('/api/inventario/unidades', data);
                    ...
                    <div className="flex min-h-full items-center justify-center p-4">
                        <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-xl transition-all">
                            <Dialog.Title as="h3" className="text-lg font-bold leading-6 text-gray-900 dark:text-white mb-4">
                                {item ? 'Editar Unidad' : 'Nueva Unidad'}
                            </Dialog.Title>
                            <form onSubmit={submit} className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Abreviatura</label>
                                    <input type="text" className="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                        value={data.abreviatura} onChange={e => setData('abreviatura', e.target.value)} required />
                                </div>
